...
var cbrtBy = require( './../lib/main.js' );
...
tape( 'the function has an arity of 7', function test( t ) {
	t.strictEqual( cbrtBy.length, 7, 'arity of 7' );
	t.end();
});
...
tape( 'the function returns a reference to the destination array', function test( t ) {
	...
	out = cbrtBy( x.length, x, 1, y, 1, accessor );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `y` unchanged', function test( t ) {
	...
	cbrtBy( -1, x, 1, y, 1, accessor );
	t.deepEqual( y, expected, 'returns `y` unchanged' );

	cbrtBy( 0, x, 1, y, 1, accessor );
	t.deepEqual( y, expected, 'returns `y` unchanged' );
...
tape( 'the function supports providing a callback execution context', function test( t ) {
	...
	cbrtBy( x.length, x, 1, y, 1, accessor, ctx );
